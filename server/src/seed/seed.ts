...
const FORCE_LIVE = process.env.SEED_FORCE_LIVE !== "0" && process.env.SEED_FORCE_LIVE !== "false";
...
// Write routes are protected by the auth guard, so the seeder must sign in first.
...
type SeedData = { feed: CommentaryEntry[]; matches: SeedMatch[] };
...
function extractCookies(res: Response): string {
  ...
  // Keep only name=value (drop Path/HttpOnly/... attributes) for the Cookie header.
  return all
    .filter(Boolean)
...
// origin must be a trusted origin (better-auth rejects missing/untrusted origins).
...
// Sign in if the seed user exists, otherwise create it. Either way capture the cookie.
async function authenticate(): Promise<void> {
  if (!SEED_EMAIL || !SEED_PASSWORD) {
    throw new Error("SEED_EMAIL and SEED_PASSWORD are required to write to protected routes.");
  }

  let response = await authRequest("sign-in/email", {
    email: SEED_EMAIL,
    password: SEED_PASSWORD,
  });

  if (response.status === 401) {
    console.log("👤 Seed user not found, creating it...");
  ...
  authCookie = extractCookies(response);
  if (!authCookie) {
    throw new Error("Auth returned no session cookie.");
  }
}
...
async function loadSeedData(): Promise<SeedData> {
  const parsed = (await readJsonFile(DEFAULT_DATA_FILE)) as
    | CommentaryEntry[]
    | { commentary?: CommentaryEntry[]; feed?: CommentaryEntry[]; matches?: SeedMatch[] };
  ...
  throw new Error("Seed data must be an array or contain a commentary/feed array.");
}
...
function isLiveMatch(match: ApiMatch): boolean {
...
async function insertCommentary(
  ...
  // Only send provided optional fields; the API rejects unknown/null fields.
  const payload: CommentaryPayload = { message: entry.message ?? "Update" };
  if (entry.minute != null) payload.minute = entry.minute;
  if (entry.sequence != null) payload.sequence = entry.sequence;
  if (entry.period != null) payload.period = entry.period;
  if (entry.eventType != null) payload.eventType = entry.eventType;
  if (entry.actor != null) payload.actor = entry.actor;
  if (entry.team != null) payload.team = entry.team;
  if (entry.metadata != null) payload.metadata = entry.metadata;
  if (entry.tags != null) payload.tags = entry.tags;
  ...
  const responsePayload = (await response.json()) as { data: { message: string } };
  return responsePayload.data;
}

function inningsRank(period?: string): number {
  if (!period) {
    return 0;
  }
...
function normalizeCricketFeed(entries: CommentaryEntry[], match: ApiMatch): CommentaryEntry[] {
  ...
  for (const key of inningsKeys) {
    ...
    const secondaryTeam = primaryTeam === match.homeTeam ? match.awayTeam : match.homeTeam;

    const neutral = inningsEntries.filter((entry) => !entry.team || entry.team === "neutral");
    const primary = inningsEntries.filter((entry) => entry.team === primaryTeam);
    const secondary = inningsEntries.filter((entry) => entry.team === secondaryTeam);
...
function cloneCommentaryEntries(
  ...
  templateMatch: ApiMatch,
  targetMatch: ApiMatch
): CommentaryEntry[] {
...
function expandFeedForMatches(
  ...
  for (const match of seedMatches) {
    if (match.id != null && !templateBySport.has(match.sport) && byMatchId.has(match.id)) {
  ...
  const expanded = [...feed];
  for (const match of seedMatches) {
    if (match.id != null && byMatchId.has(match.id)) {
      continue;
    }
    const templateMatch = templateBySport.get(match.sport);
    if (!templateMatch || templateMatch.id == null) {
      continue;
    }
    const templateEntries = byMatchId.get(templateMatch.id) || [];
    // cloneCommentaryEntries needs ApiMatch shape; seed matches carry the same fields.
    expanded.push(
      ...cloneCommentaryEntries(templateEntries, templateMatch as ApiMatch, match as ApiMatch)
    );
  }
...
function buildRandomizedFeed(
  ...
  while (randomized.length < feed.length) {
    const candidates = matchIds.filter((id) => (buckets.get(id) || []).length > 0);
    ...
    lastMatchId = typeof choice === "number" ? choice : null;
  }
...
function toMatchState(match: ApiMatch): MatchState {
  return {
    match,
...
async function seed(): Promise<void> {
  ...
  for (const match of matchesList) {
    ...
    matchMap.set(match.id, toMatchState(match));
  }
  ...
  if (Array.isArray(seedMatches) && seedMatches.length > 0) {
    for (const seedMatch of seedMatches) {
      ...
      if (Number.isInteger(seedMatch.id)) {
        matchMap.set(seedMatch.id!, toMatchState(match));
      }
      matchMap.set(match.id, toMatchState(match));
    }
  }
  ...
  for (const entry of randomizedFeed) {
    ...
    if (!target) {
      console.warn("Skipping entry: matchId missing or not found:", entry.message);
      continue;
    }
